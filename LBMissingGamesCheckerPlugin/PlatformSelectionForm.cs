    ...
    public partial class PlatformSelectionForm : Form
    {
        ...
        private void createLaunchBoxWishlistPlaylistToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Create a fresh shadow platform
                ...
                //// --- Category Management & IParent Pruning ---
                //if (shouldNestUnderCategory)
                //{
                //    // Ensure root "Wishlists" Platform Category exists
                //    var wishlistCategory = PluginHelper.DataManager.GetAllPlatformCategories()
                //        .FirstOrDefault(c => c.Name.Equals(targetCategoryName, StringComparison.OrdinalIgnoreCase));
                ...
                //    // Assign primary Category property
                //    shadowPlatform.Category = targetCategoryName;
                //    DebugTxt($"Set '{shadowPlatformName}' Category property to '{targetCategoryName}'.");
                //}
                //else
                //{
                //    // Clear Category property
                //    shadowPlatform.Category = string.Empty;
                //    DebugTxt($"Cleared Category property for '{shadowPlatformName}'.");
                //}

                //// Fetch IParent relationship records created globally for this platform
                //var existingParentRecords = PluginHelper.DataManager.GetAllParents()
                //    .Where(p => !string.IsNullOrEmpty(p.PlatformName) &&
                //                p.PlatformName.Equals(shadowPlatformName, StringComparison.OrdinalIgnoreCase))
                //    .ToList();

                //if (shouldNestUnderCategory)
                //{
                //    // Prune auto-assigned hardware categories (e.g., Handhelds or Consoles auto-injected by ScrapeAs)
                //    foreach (var parentRecord in existingParentRecords)
                //    {
                //        if (!string.Equals(parentRecord.ParentPlatformCategoryName, targetCategoryName, StringComparison.OrdinalIgnoreCase))
                //        {
                //            PluginHelper.DataManager.TryRemoveParent(parentRecord);
                //            DebugTxt($"Pruned auto-assigned parent record: '{parentRecord.Id}'");
                //        }
                //    }
                //}
                //else
                //{
                //    // Prune the 'Wishlists' parent record specifically
                //    foreach (var parentRecord in existingParentRecords)
                //    {
                //        if (string.Equals(parentRecord.ParentPlatformCategoryName, targetCategoryName, StringComparison.OrdinalIgnoreCase))
                //        {
                //            PluginHelper.DataManager.TryRemoveParent(parentRecord);
                //            DebugTxt($"Pruned 'Wishlists' parent record: '{parentRecord.Id}'");
                //        }
                //    }
                //}
