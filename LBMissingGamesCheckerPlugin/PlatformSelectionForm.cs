    ...
    public partial class PlatformSelectionForm : Form
    {
        ...
        private void createLaunchBoxWishlistPlaylistToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    shadowPlatform.Category = string.Empty;
                    DebugTxt($"Cleared Category property for '{shadowPlatformName}'.");
                }
                ...
                // In-Memory Parent Dictionary Pruning
                try
                {
                    var dataManagerInstance = PluginHelper.DataManager;
                    var getParentsMethod = dataManagerInstance.GetType().GetMethod("get_Parents",
                        System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);

                    if (getParentsMethod != null)
                    {
                        var parentsDict = getParentsMethod.Invoke(dataManagerInstance, null) as System.Collections.IDictionary;
                        ...
                        if (parentsDict != null)
                        {
                            ...
                            if (shouldNestUnderCategory)
                            {
                                // Remove hardware category links (e.g., 'Sega 32X Wishlist||||Consoles|')
                                foreach (System.Collections.DictionaryEntry entry in parentsDict)
                                {
                                    string keyStr = entry.Key?.ToString() ?? "";
                                    if (keyStr.StartsWith($"{shadowPlatformName}||||", StringComparison.OrdinalIgnoreCase) &&
                                       !keyStr.EndsWith($"||||{targetCategoryName}|", StringComparison.OrdinalIgnoreCase))
                                    {
                            ...
                            {
                                // Remove the Wishlists link (e.g., 'Sega 32X Wishlist||||Wishlists|')
                                foreach (System.Collections.DictionaryEntry entry in parentsDict)
                                {
                                    string keyStr = entry.Key?.ToString() ?? "";
                                    if (keyStr.StartsWith($"{shadowPlatformName}||||", StringComparison.OrdinalIgnoreCase) &&
                            ...
                            foreach (var key in keysToRemove)
                            {
                                ...
                                DebugTxt($"Pruned Parent Dictionary Key: '{key}'");
                            }

                            // Force DataManager to flag parents as modified so changes persist to disk on Save()
                            var setParentsChangedMethod = dataManagerInstance.GetType().GetMethod("set_ParentsChanged",
                                System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                            setParentsChangedMethod?.Invoke(dataManagerInstance, new object[] { true });
                        }
                    }
                }
                catch (Exception parentEx)
                {
                    DebugTxt($"Warning: Dictionary pruning encountered an issue: {parentEx.Message}");
                }
